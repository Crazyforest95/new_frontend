...
function InterviewTimeSelector({ data, onChange }) {
  // 序列化数据
    ...
    return (
      <div key={item.date}>
        ...
        {item.disable ? (
          ...
          <>
            {['am', 'pmA', 'pmB'].map(t => {
              if (item[t] === 'disable' || item[t] === undefined) {
                return <Block bgColor="#dedede" />;
              }
...
export default InterviewTimeSelector;
