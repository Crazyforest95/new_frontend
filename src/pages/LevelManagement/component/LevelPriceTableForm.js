        ...
        render: (text, record) => {
          if (record.editable) {
            return (
              <Input
                value={text}
                onChange={e => this.handleFieldChange(e.target.value, 'level', record.key)}
        ...
        render: (text, record) => {
          if (record.editable) {
            return (
              <Input
                value={text}
                onChange={e => this.handleFieldChange(e.target.value, 'price', record.key)}
                onKeyPress={e => this.handleKeyPress(e.target.value, record.key)}
                placeholder="请输入价格"
                style={{ width: '100%' }}
                addonAfter={`元/${way}`}
